...
const T3Status = (props) => {
  ...
  return (
    <div className="t3-status row text-center">
      <h4 className={'column small-2' + (currentPlayer !== null && currentPlayer === player1 ? ' status-avatar-active' : '')}>{player1.avatar}</h4>
      <h4 className="column small-8">{renderDifficulty()}</h4>
      <h4 className={'column small-2' + (currentPlayer !== null && currentPlayer === player2 ? ' status-avatar-active' : '')}>{player2.avatar}</h4>
    </div>
  );
}
